...
ulimit -v 16000000  # Setar limite para execucao
...
# Loop externo
for ((execucao = 1; execucao <= QTD_EXEC; execucao++)); do

    # Cria pasta para a iteração
    output_dir="data/output/Execucao${execucao}"
    mkdir -p "$output_dir"
    ...
    echo "ITERACAO;INSTANCIA_NOME;TAM_POPULACAO;QTD_TURMAS_HEURISTICA;QTD_REPT_HEURISTICA;PESO_JANELA;PESO_SEXTO;TEMPO_EXEC;ID_SOLUCAO;JANELA_SOLUCAO;SEXTO_HORARIO_SOLUCAO;VALOR_SOLUCAO" >> "$output_dir/bash-time.csv"

    for instancia_value in "${INSTANCIAS_LIST[@]}"; do
        output_dir_iteration="$output_dir/$instancia_value"
            ...
            for qtd_rept_value in "${REPTS_LIST[@]}"; do
                for peso_janela_value in "${PESOS_JANELA_LIST[@]}"; do
                    for peso_sexto_value in "${PESOS_SEXTO_LIST[@]}"; do
                        ...
                        subfolder_path="$output_dir_iteration/$subfolder_name"
                        ...
                        if { time -p (./test/exe "$instancia_value" "$TAM_POPULACAO" "$qtd_turma_value" "$qtd_rept_value" "$peso_janela_value" "$peso_sexto_value") ; } 2> >(grep real | awk '{print $2}' > "$output_dir_iteration/time.txt") > /dev/null ; then
                            ...
                            exec_time=$(cat "$output_dir_iteration/time.txt")
                            echo "$progresso_atual;$instancia_value;$TAM_POPULACAO;$qtd_turma_value;$qtd_rept_value;$peso_janela_value;$peso_sexto_value;$exec_time;$best_id;$best_window_value;$best_sixth_grade_value;$solution_value" >> "$output_dir_iteration/bash-time.csv"
                        else
                            ((progresso_atual++))
                            ...
                            exec_time=$(cat "$output_dir_iteration/time.txt")
                            echo "$progresso_atual;$instancia_value;$TAM_POPULACAO;$qtd_turma_value;$qtd_rept_value;$peso_janela_value;$peso_sexto_value;RUNTIME_ERROR;RUNTIME_ERROR;RUNTIME_ERROR;RUNTIME_ERROR" >> "$output_dir_iteration/bash-time.csv"
                        fi
                    done
                done
            done
    ...
    mv "$output_dir_iteration/bash-time.csv" "$output_dir/"
    echo ""
    echo "Programa encerrado com $erros! Resultados salvos em $output_dir/"
    echo ""
